...
namespace WindowsAPI.USB
{
    ...
    public class Hid : object
    {
        ...
        /// <returns>获取成功true</returns>
        public static bool HID_SetupDiEnumDeviceInterfaces(IntPtr hidInfoSet, IntPtr deviceInfoData, ref Guid interfaceClassGuid, UInt32 memberIndex, ref SP_DEVICE_INTERFACE_DATA deviceInterfaceData) {
            bool flag = false;
            flag = SetupDiEnumDeviceInterfaces(hidInfoSet, IntPtr.Zero, ref interfaceClassGuid, memberIndex, ref deviceInterfaceData);
            // 获得最后一次获得的错误
        ...
        public static void GetHidDeviceList(ref List<string> deviceList)
        {
            ...
            //IntPtr hidInfoSet = SetupDiGetClassDevs(ref hUSB, 0, IntPtr.Zero, DIGCF.DIGCF_ALLCLASSES);
            if (hidInfoSet != IntPtr.Zero)
            {
                ...
                //interfaceInfo.cbSize = 32;
                //查询集合中每一个接口
                for (index = 0; index < MAX_USB_DEVICES; index++)
                {
                    int lastErrorCode2 = Marshal.GetLastWin32Error();
                    //得到第index个接口信息
                    
                    if (HID_SetupDiEnumDeviceInterfaces(hidInfoSet, IntPtr.Zero, ref hUSB, index, ref interfaceInfo))
                    {
                        int buffsize = 0;
        ...
        /// </summary>
        /// <param name="ClassGuid">GUID for a device setup class or a device interface class. 
        /// 一个指向GUID的指针，此GUID可标识一个设备安装类或一个设备接口类。这个指针是可选的，并且可以为NULL
        /// </param>
        /// <param name="Enumerator">A pointer to a NULL-terminated string that supplies the name of a PnP enumerator or a PnP device instance identifier. 
        ...
        /// </param>
        /// <param name="HwndParent">A handle of the top-level window to be used for a user interface
        ...
        /// Flags是一个DWORD类型的变量，通过此参数来过滤指定的设备信息集中的设备。此参数可以是以下标志位中的一个或多个的按位或组合，更多有关标志位组合的信息，
        /// </param>
        ...
        /// <summary>
        /// 创建打开文件或者串口 设备 This function creates, opens, or truncates a file, COM port, device, service, or console. 
        /// </summary>
    ...
    public enum DIGCF
    {
        DIGCF_DEFAULT = 0x00000001, // 对于指定的设备接口类，只返回与系统默认设备接口相关联的设备（如果已设置的话）。        
        DIGCF_PRESENT = 0x00000002, //只返回当前系统中存在的（已连接）设备。
        DIGCF_ALLCLASSES = 0x00000004, //返回所有已安装设备的列表或所有设备接口类。
        DIGCF_PROFILE = 0x00000008, //只返回当前硬件列表中的一部分设备。
        DIGCF_DEVICEINTERFACE = 0x00000010 //返回支持指定设备接口类的设备。如果Enumerators参数制定了设备的实例ID，那么必须在Flags参数中设置此标志位。
    }
    /// <summary>
